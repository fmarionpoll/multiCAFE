...
import plugins.fmp.multicafe2.tools.nherve.image.segmentation.Segmentation;
import plugins.fmp.multicafe2.tools.nherve.toolbox.image.BinaryIcyBufferedImage;
import plugins.fmp.multicafe2.tools.nherve.toolbox.image.DifferentColorsMap;
...
public class MaskStack implements Iterable<Mask>, Serializable
{
    ...
    {
        Mask m2 = createNewMask("Copy of (" + m.getId() + ") " + m.getLabel(), m.isNeedAutomaticLabel(), m.getColor(),
                m.getOpacity());
        if (m.hasBinaryData())
        {
    ...
    {
        Mask m2 = createNewMask("Copy of (" + m.getId() + ") " + m.getLabel(), m.isNeedAutomaticLabel(), colorMap,
                m.getOpacity());
        if (m.hasBinaryData())
        {
            m2.setBinaryData(m.getBinaryData().getCopy());
    ...
    public Mask createNewMask(String label, boolean needAutomaticLabel, DifferentColorsMap colorMap, float opacity)
            throws MaskException
    {
        Mask m = new Mask(width, height, label, false);
        m.setOpacity(opacity);
     ...
     *        the new active id
     */
    public void setActiveIndex(int activeIdx)
    {
